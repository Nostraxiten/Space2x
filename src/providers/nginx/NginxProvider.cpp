...
core::Result<core::ServiceState> NginxProvider::detectState(const core::Engine& engine) const {
    const auto serviceName = m_manifest.serviceNameForCurrentPlatform();
    return engine.serviceManager().getState(serviceName);
}

core::Result<void> NginxProvider::performHealthCheck(const core::Engine& engine) const {
